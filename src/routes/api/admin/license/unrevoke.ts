...
// POST /api/admin/license/unrevoke
// Body: { license_key: string } OR { user_id: string }
// Header: Authorization: Bearer <LICENSE_ADMIN_SECRET>
export const Route = createFileRoute("/api/admin/license/unrevoke")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        await db.update(licenses).set({ revokedAt: null }).where(eq(licenses.id, rows[0]!.id));
